...
impl<D: DrawTarget<Color = Rgb888>> Odys<D> {
    ...
    fn clear_screen(&mut self) {
        ...
        surface.clear(Rgba8888UNORM::RED);
    }

    fn redraw_all(&mut self) {
        self.clear_screen();
        serial_println!("Odys: redraw_all - begin");
        for i in 0..=self.curr_line_idx {
            if !self.lines[i].is_empty() {
                ...
                serial_println!("Odys: redraw_all - line {}: {}", i, self.lines[i].as_str());
            }
        }
    }
...
impl<D: DrawTarget<Color = Rgb888>> Write for Odys<D> {
    fn write_str(&mut self, s: &str) -> core::fmt::Result {
        Odys::write_str(self, s);
        Ok(())
    }
}
...
pub extern "C" fn main() -> ! {
    let window_id = unsafe { rustspace::sys_create_window(600, 400, 700, 50) };
    ...
    rustspace::println!(
        "Odys: window {}x{} id={} mapped at {:p}, front: {:p}",
        width,
        height,
        window_id,
        pixels,
    ...
    let mut terminal = Odys::new(surface, window_id);

    rustspace::println!("Odys: writing");

    terminal.write_line("Odys");
    terminal.write_line("=======================");

    rustspace::println!("Odys: starting loop");

    let mut window_info = WindowInfo::zeroed();
    let ok = unsafe { sys_get_window_info(window_id, &mut window_info) };
    rustspace::println!(
        "odys: window info: {}x{} event_buffer_vaddr: {}",
        window_info.width,
        window_info.height,
    ...
    loop {
        loop {
            ...
            // if terminal.window_id != focused_window_id {
            //     rustspace::println!(
            //         "odys: focused_window_id: {}, this id: {}",
            //         focused_window_id,
            //         terminal.window_id
            //     );
            //     break;
        ...
        //rustspace::println!("Odys: loop - begin");
        ...
        //rustspace::println!("Odys: loop - end");
    }
