...
use crate::gfx::color::{Rgba8888F, Rgba8888UNORM};
use crate::gfx::pipeline::{
    PSIn, PixelShader, VSIn, VSOut, VSOut3D, Vertex3D, VertexShader, VertexShader3D,
};
use crate::gfx::resources::ConstantBuffer;
use crate::gfx::transform::{F32x4Ext, Matrix4x4};
...
impl PixelShader for UVDebugPS {
    fn run(&self, input: &mut PSIn) {
        ...
        input.render_target[0] = Rgba8888UNORM::from_rgbf32(u, v, 0.0).to_u32_xrgb();
    }
}
...
impl PixelShader for TextureSamplePS {
    fn run(&self, input: &mut PSIn) {
        if let Some(texture) = input.textures.get(self.texture_slot) {
            let u = input.attributes[0];
            let v = input.attributes[1];
            let color = Rgba8888UNORM::from_rgbf32(u, v, 0.0);
...
pub struct BlinnPhongPS {
    ...
    pub specular_color: Rgba8888F,
    pub shininess: f32,
    pub ambient_color: Rgba8888F,
    pub camera_pos: f32x4,
}
...
impl PixelShader for BlinnPhongPS {
    fn run(&self, input: &mut PSIn) {
        ...
        let r = self.ambient_color.r + self.albedo.r * diffuse + self.specular_color.r * specular;
        let g = self.ambient_color.g + self.albedo.g * diffuse + self.specular_color.g * specular;
        let b = self.ambient_color.b + self.albedo.b * diffuse + self.specular_color.b * specular;

        input.render_target[0] = Rgba8888F::from_rgbf32(r, g, b).to_u32_xrgb();
    }
}
